...
def run_hybrid_trial(
    ...
    try:
        ...
        with capture_ctx:
            ...
            client_id = f"hybrid-trial-{trial_id}-{os.urandom(4).hex()}"
            c = mqtt.Client(
        ...
        cpu_end = _cpu_time_ns()
        logger.error("Hybrid trial %d failed — %s: %s", trial_id, type(exc).__name__, exc)
        raise
