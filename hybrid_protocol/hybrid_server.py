...
def _handle_connection(conn: socket.socket) -> None:
    try:
        ...
        lcg = LCG(os.urandom(8))
        scalar_bytes = lcg.next_bytes(32)
        private_value = int.from_bytes(scalar_bytes, "big")
        if private_value == 0:
            private_value = 1
        server_key = ec.derive_private_key(private_value, SECP256R1())
        server_pub = server_key.public_key()
        q_s_bytes = server_pub.public_bytes(Encoding.X962, PublicFormat.UncompressedPoint)

        n_s = lcg.next_bytes(_NONCE_LEN)
        ...
        k_enc, k_mac = derive_keys(shared_secret, n_c, n_s, "hybrid")
        ...
        logger.info("Hybrid handshake complete")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Hybrid ECC-LCG auth server")
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as srv:
        ...
        srv.listen(16)
        logger.info("Hybrid ECC-LCG auth server listening on 127.0.0.1:%d", args.port)
        while True:
            try:
